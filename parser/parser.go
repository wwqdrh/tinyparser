...
// primary → → "true" | "false" | "nil" | "this" | "[", expression, ... , "]"
...
func (p *Parser) primary() (expr ast.Expression, err error) {
	...
	if p.match(token.LeftSquare) {
		...
		// [] 空列表
		_, err = p.consume(token.RightSquare)
		if err == nil {
			expr = &ast.ListExpr{Values: values, Idx: idx}
			return
		}
		...
		for {
			...
			// if have a index
			_, err = p.consume(token.LeftSquare)
			if err == nil {
				idx, err = p.consume(token.Number)
				if err != nil {
					return
				}
	...
	if p.match(token.LeftBrace) {
		values := map[interface{}]ast.Expression{}
		...
		for {
			...
			val, ok := expr.(*ast.AssignExpr)
			if !ok {
				err = fmt.Errorf("the dict var must have a assign: %w", err)
				return
			}

			values[val.Name.Lexeme] = val.Value
		...
		expr = &ast.DictExpr{Values: values}
		return
	}
...
// call  → primary ( "(" arguments? ")" | "." IDENTIFIER | "[" idx "]" )* ;
func (p *Parser) call() (expr ast.Expression, err error) {
	...
	for {
		if p.match(token.LeftParen) {
		...
		} else if p.match(token.LeftSquare) {
			var slicename *token.Token
			...
			case *ast.GetExpr:
				slicename = e.Name
			default:
				err = fmt.Errorf("the [] only for slice and dict data")
				return
			}

			var idx *token.Token
			if idx == nil {
				idx, _ = p.consume(token.Number)
			}
			if idx == nil {
				idx, _ = p.consume(token.String)
			}
			if idx == nil {
				err = fmt.Errorf("the [] action's index must be a string or a number")
				return
			}

			_, err = p.consume(token.RightSquare)
			if err != nil {
				err = fmt.Errorf("the slice action must end with ]")
				return
			}

			expr = &ast.IndexExpr{Object: slicename, Idx: idx}
		} else {
			break
		}
	}
