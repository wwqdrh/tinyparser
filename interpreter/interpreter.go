...
func (i *Interpreter) DefineEnv(name string, val interface{}) {
...
func (i *Interpreter) VisitDictExpr(v *ast.DictExpr) (result interface{}, err error) {
	res := map[interface{}]interface{}{}

	for name, value := range v.Values {
		goval, err := i.evaluate(value)
		if err != nil {
...
func (i *Interpreter) VisitListExpr(v *ast.ListExpr) (result interface{}, err error) {
	if v.Idx == nil {
		res := []interface{}{}

		for _, val := range v.Values {
			goval, err := i.evaluate(val)
			if err != nil {
				logger.DefaultLogger.Warn(err.Error())
				continue
			}
			res = append(res, goval)
		}
		return res, nil
	} else {
		var idxint int
		switch v.Idx.Type {
		case token.Number:
			idx, ok := v.Idx.Literal.(float64)
			if !ok {
				err = fmt.Errorf("the list slice must be a number")
				return
			}
			idxint = int(idx)
		default:
		...
		resval, err := i.evaluate(v.Values[idxint])
		if err != nil {
			return nil, err
		}
		return resval, nil
...
func (i *Interpreter) VisitIndexExpr(v *ast.IndexExpr) (result interface{}, err error) {
	result, err = i.indexList(v)
	if err == nil {
		return
	}

	result, err = i.indexDict(v)
	if err == nil {
		return
	}
...
func (i *Interpreter) indexList(v *ast.IndexExpr) (result interface{}, err error) {
	var data interface{}
	data, err = i.lookUpVariable(v.Object, v)

	idx, ok := v.Idx.Literal.(float64)
	if !ok {
		err = fmt.Errorf("list slice idx must be a number")
		return
	}
	...
	case []interface{}:
		...
		return
	default:
		err = fmt.Errorf("the slice action must be a slice")
		return
	}
}

func (i *Interpreter) indexDict(v *ast.IndexExpr) (result interface{}, err error) {
	var data interface{}
	data, err = i.lookUpVariable(v.Object, v)
	idx, ok := v.Idx.Literal.(string)
	if !ok {
		err = fmt.Errorf("dict idx must be a string")
		return
	}

	switch datadict := data.(type) {
	...
	case map[string]interface{}:
		...
		return
	default:
		err = fmt.Errorf("the slice action must be a dict")
		return
	}
}
...
func (i *Interpreter) VisitGetExpr(v *ast.GetExpr) (result interface{}, err error) {
	...
	case map[string]interface{}:
		result = value[v.Name.Lexeme]
		return
	case *Instance:
		return value.Get(v.Name)
	default:
